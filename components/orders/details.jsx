...
const ProofModal = ({ order }) => {
	...
	return (
		...
		>
			<Image my={-6} alt="proof" src={order.proof} />
		</Modal>
	)
}
...
const ReviewModal = ({ order }) => {
	...
	const addMutation = useMutation((data) => api.create('/reviews', data), {
		onSuccess: () => {
			...
			toast({
				position: 'top',
				duration: 1000,
				render: () => <Toast title="Success" description="Order rated successfully." />
			})
		}
	})
	...
	return (
		<Modal
			title="Rate & Review"
			toggle={(onOpen) => (
				<Button size="lg" colorScheme="brand" leftIcon={<FiStar size={16} />} onClick={onOpen}>
					Rate & Review
				</Button>
			)}
			disclosure={disclosure}
			...
			<form onSubmit={handleSubmit(onSubmit)}>
				<Flex direction="column" gap={6}>
					<chakra.div position="relative">
						{image && <IconButton position="absolute" top={3} right={3} borderRadius="full" size="xs" colorScheme="brand" zIndex={1} icon={<FiX size={12} />} onClick={() => setImage(null)} />}

						{image ? (
							<AspectRatio ratio={1}>
								<Image borderRadius={12} alt="product" src={typeof image === 'object' ? URL.createObjectURL(image) : image} />
							</AspectRatio>
						) : (
							<AspectRatio ratio={1}>
								<Flex bg="brand.alpha" position="relative" align="center" direction="column" gap={2} border="1px dashed" borderColor="brand.default" borderRadius={12} px={6} py={12}>
									<chakra.input type="file" position="absolute" top={0} left={0} h="full" w="full" opacity={0} cursor="pointer" onChange={handleImage} />
									<Icon as={FiPlus} boxSize={6} color="brand.default" />
					...
					<FormControl isInvalid={errors.reviews}>
						<Textarea placeholder="Write a message" size="lg" minH={220} {...register('reviews', { required: true })}></Textarea>
...
const ViewReviewModal = ({ order }) => {
	...
	const search = isReviewsFetched ? reviews.filter((review) => review.order.id === order._id) : null
	const review = search ? search[0] : null
	const [isLoading, setIsLoading] = useState(false)

	const deleteMutation = useMutation((data) => api.remove('/reviews', data.id), {
		onSuccess: () => {
	...
	return (
		<Modal
			title="Rate & Review"
			toggle={(onOpen) => (
				<Button display={!review ? 'none' : 'flex'} size="lg" colorScheme="brand" leftIcon={<FiStar size={16} />} disabled={!order.reviews.status} onClick={onOpen}>
					Product Review
				</Button>
			)}
			disclosure={disclosure}
				...
				<Flex direction="column" gap={6}>
					<Flex justify="center" align="center">
						{[...Array(review.ratings)].map((data, index) => (
							<Icon as={FiStar} boxSize={6} fill="brand.default" color="brand.default" key={index} />
						))}

						{[...Array(5 - review.ratings)].map((data, index) => (
							<Icon as={FiStar} boxSize={6} color="brand.default" key={index} />
						))}
					</Flex>
					...
					<Image borderRadius={12} alt="review-image" src={review.image} />

					<Button size="lg" colorScheme="red" isLoading={isLoading} onClick={() => onDelete(review._id)}>
						Delete Review
					</Button>
				</Flex>
			) : (
...
const Details = ({ session, order }) => {
	...
	const editMutation = useMutation((data) => api.update('/orders', order._id, data), {
		onSuccess: () => {
			queryClient.invalidateQueries('orders')
			setIsLoading(false)
			...
			<Flex direction="column" gap={6}>
				...
				<Flex direction="column" gap={3}>
					...
					<Flex justify="space-between" align="center" gap={6}>
						...
						<Text fontSize="sm" fontWeight="medium" color="accent-1">
							{month[order.created.split(',')[0].split('/')[0] - 1] + ' ' + order.created.split(',')[0].split('/')[1] + ', ' + order.created.split(',')[0].split('/')[2]}
						</Text>
					</Flex>
				...
				<Flex direction="column" gap={3}>
					...
					{order.method === 'GCash' && <ProofModal order={order} />}
						...
						<>
							{order.ship.status ? (
								order.completed.status ? (
									...
									<Button size="lg" colorScheme="brand" disabled={!order.receive.status} isLoading={isLoading} onClick={onReceive}>
										Orders Received
									</Button>
								)
							) : order.cancelled.status ? null : (
								<>
									<Button size="lg" colorScheme="brand" isLoading={isLoading} onClick={onAccept}>
										Accept Orders
									</Button>

									<Button variant="outline" size="lg" colorScheme="brand" isLoading={isLoading} onDoubleClick={onCancel}>
										Cancel Orders
									</Button>
								</>
							)}
					...
					{session.user.role === 'Customer' &&
						(order.ship.status ? (
							...
							<Button size="lg" colorScheme="brand" isLoading={isLoading} onClick={onCancel}>
								Cancel Orders
							</Button>
						))}
				</Flex>
