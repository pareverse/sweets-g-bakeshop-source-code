						...
						<Tbody>
							{order.items.map((item) => (
								<Tr key={item._id}>
									...
									<Td>
										<Text>₱{(item.product.price * item.quantity * (item.product.discount.percentage / 100)).toFixed(2)}</Text>
									</Td>

									<Td>
										<Text>₱{(item.product.price * item.quantity - item.product.price * item.quantity * (item.product.discount.percentage / 100)).toFixed(2)}</Text>
									</Td>
								</Tr>
							))}
						</Tbody>
