...
const Statistics = ({ users, isUsersFetched, products, isProductsFetched, orders, isOrdersFetched, sales, completed, reports }) => {
	return (
		<>
			...
			<GridItem colSpan={{ base: 12, lg: 6 }}>
				<Card>
					<Flex justify="space-between" align="center">
						<Flex direction="column" gap={1} w="calc(100% - 76px)">
							<Text fontSize="2xl" fontWeight="semibold" color="accent-1" noOfLines={1}>
								{sales ? (sales === 'daily' ? currency(reports('daily', completed).sales) : sales === 'monthly' ? currency(reports('monthly', completed).sales) : sales === 'yearly' && currency(reports('yearly', completed).sales)) : currency(reports('gross', completed).sales)}
							</Text>
