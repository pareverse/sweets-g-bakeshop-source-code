...
import { Avatar, Badge, Container, Flex, IconButton, Select, Td, Text, Tr } from '@chakra-ui/react'
			...
			<Flex direction="column" gap={6}>
				...
				<Card>
					<Table
						...
						th={['Full Name', 'Email', 'Orders', 'Role', 'Status', 'Joined', '']}
						td={(user) => (
							<Tr key={user._id}>
								<Td>
								...
								<Td>
									<Badge variant="tinted" colorScheme={user.role === 'Admin' ? 'yellow' : 'blue'}>
										{user.role}
									</Badge>
								</Td>
								...
								<Td>
									<Text>{month[user.created.split(',')[0].trim().split('/')[0] - 1] + ' ' + user.created.split(',')[0].trim().split('/')[1] + ', ' + user.created.split(',')[0].trim().split('/')[2]}</Text>
								</Td>
