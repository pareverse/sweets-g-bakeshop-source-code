...
const Order = () => {
	...
	const { data: order, isFetched: isOrderFetched } = useQuery(['order', id], () => api.get('/orders', id))
			...
			<Flex direction="column" gap={6}>
				...
				<Grid templateColumns={{ base: '1fr', lg: '1fr 384px' }} alignItems="start" gap={6}>
					...
					<GridItem display="grid" gap={6}>
						...
						<Status order={order} />
						{session.user.role === 'Admin' && <Customer order={order} />}
						<Address order={order} />
						{!order.cancelled.status && <Courier session={session} order={order} />}
					</GridItem>
				</Grid>
			</Flex>
		</Container>
